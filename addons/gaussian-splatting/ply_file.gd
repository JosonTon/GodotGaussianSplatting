...
static func load_gaussian_splats(point_cloud, stride : int, output : PackedFloat32Array, should_terminate_reference : Array[bool], num_points_loaded : Array[int], center_at_origin : bool, callback : Callable):
	...
	var task_id = WorkerThreadPool.add_group_task(func(i : int):
		...
		var base_offset := i*stride*STRUCT_SIZE
		for j in tile_size:
			var v = num_propoerties*(i*stride + j) # Vertex index
			var b := base_offset + j*STRUCT_SIZE     # Output index

			### Position ###
			for k in range(3):  output[b+k+0] = p[v+k+0]
			output[b+3] = creation_time
			...
			output[b+0+4] = cov_3d.x[0]
			output[b+1+4] = cov_3d.y[0]
			output[b+2+4] = cov_3d.z[0]
			output[b+3+4] = cov_3d.y[1]
			output[b+4+4] = cov_3d.z[1]
			output[b+5+4] = cov_3d.z[2]

			### Opacity ###
			output[b+6+4] = 1.0 / (1.0 + exp(-p[v+54]))

			### Spherical Harmonic Coefficients ###
			for k in range(3): output[b+k+12] = p[v+k+6]
			for k in range(0, 45, 3):
				output[b+(k+0)+15] = p[v+(k/3+ 0)+9]
				output[b+(k+1)+15] = p[v+(k/3+15)+9]
				output[b+(k+2)+15] = p[v+(k/3+30)+9]
		if should_terminate_reference[0]: return
		mutex.lock()
	...
	# Centroid centering
	if center_at_origin and point_cloud.size > 0:
		var cx := 0.0; var cy := 0.0; var cz := 0.0
		for i in range(point_cloud.size):
			var b := i * STRUCT_SIZE
			cx += output[b]; cy += output[b+1]; cz += output[b+2]
		cx /= point_cloud.size; cy /= point_cloud.size; cz /= point_cloud.size
		for i in range(point_cloud.size):
			var b := i * STRUCT_SIZE
			output[b] -= cx; output[b+1] -= cy; output[b+2] -= cz
