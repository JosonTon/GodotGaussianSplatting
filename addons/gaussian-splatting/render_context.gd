...
static func create(device : RenderingDevice=null) -> Object:
...
func create_pipeline(block_dimensions : Array, descriptor_sets : Array, shader : RID) -> Callable:
	...
	return func(context, compute_list : int, push_constant : PackedByteArray=[], descriptor_set_overwrites:=[], block_dimensions_overwrite_buffer:=RID(), block_dimensions_overwrite_buffer_byte_offset:=0) -> void:
